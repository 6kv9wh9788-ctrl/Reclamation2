...
## Video-driven tuning revision

The recorded fight showed a missed opening sweep followed by a full counterattack opening, then a bite interrupted by another civilian. This revision makes the brute commit only against a closer crowd, introduces a dexterity-based reaction delay, and distinguishes missed-sweep recovery from a connected sweep. It does not increase health or guarantee the brute wins.

The recording also exposed `Script error (CombatDirector): Start() can not take parameters.` The private action helper is now named BeginAction, avoiding Unity's reserved Start lifecycle message. Clear the Console after importing the patch, then confirm the error does not recur on a new Play session.
...
| Sweep trigger | At least two standing, reachable, unobstructed humans within 1.7 metres, and the sweep is ready |
| Sweep warning | 1.0 simulation second; facing locks at the start, so it does not track a dodging target |
...
| Connected-sweep recovery | 1.7 simulation seconds, with 25% extra incoming damage; hits do not shorten this recovery |
| Missed-sweep recovery | 0.65 simulation seconds, with normal incoming damage |
| Repeat timing | 6 simulation seconds from the start of a connected sweep; a miss allows another attempt after 2 more seconds; an interrupted windup can be retried after stagger |
...
- Ready survivors first read the sweep windup (0.35 simulation seconds for the civilian preset, 0.12 for the veteran), then attempt a stamina-costing dodge beyond its reach. Those already outside the forward danger area wait for the opening instead of immediately charging back in.
...
The tuning revision adds tests for close-range commitment, missed recovery without bonus damage, and the Unity lifecycle method collision. Existing sweep tests now use the closer engagement distance and verify that reaction time pauses with the simulation. Apply the follow-up patch to the original brute checkpoint, rerun both suites, and reuse your saved Three Civilians vs Brute scene.
